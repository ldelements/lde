...
- **Binary media is rejected without downloading.** A response whose `Content-Type` announces a binary media asset – `image/*`, `audio/*`, or `video/*` – yields `binary-content` and the body is never read: the stream is cancelled, so a `schema:contentUrl` that dereferences to a full-resolution image or video does not get buffered just to fail JSON parsing. A missing or ambiguous type (`text/plain`, `application/octet-stream`) is **not** treated as binary, so a manifest served with an odd type is still parsed.
