...
[`Writer`](https://github.com/ldelements/lde/blob/main/packages/pipeline/src/writer/writer.ts)s. Shapes can be provided in any
...
The validator opens one long-lived run per writer via `Writer.openRun()` –
with a fabricated run context, since the report stream is the validator’s
own run rather than the pipeline’s – and streams each batch to it via
`RunWriter.write(dataset, quads)`. Each run’s `RunWriter.flush(dataset,
'success')` is invoked from `ShaclValidator.report(dataset)` – i.e. once the
pipeline finishes a dataset.

When results were written, `validate()` returns a `message` of the form
`Report sent to N writer(s)`; with `onInvalid: 'halt'` the pipeline surfaces
this message in the halt error, pointing at where the report landed.

A `validate()` call with an empty quad batch short-circuits to
`{ conforms: true, violations: 0 }` without touching the SHACL engine.
...
For convenience, the package re-exports the `SerializationFormat` type (and
the `Validator`, `ValidationReport` and `ValidationResult` types) from
`@lde/pipeline`.
