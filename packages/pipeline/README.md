...
For pipelines that run without a triplestore, `FileProvenanceStore` persists all records to a single JSON file, keyed by dataset URI:

```typescript
import { FileProvenanceStore } from '@lde/pipeline';
